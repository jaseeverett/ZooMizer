...
mf.params@mu_b <- readRDS("data/mu_b.RDS")
...
new_Encounter <- function(params, n, n_pp, n_other, t, ...) {
    ...
    # In this case we use the code from mizer version 0.3
        ...
        encounter <- temp_eff * params@search_vol * (phi_prey_species + phi_prey_background)
...
new_EReproAndGrowth <- function (params, n, n_pp, n_other, t, encounter, feeding_level, 
    ...) 
{
    sweep((1 - feeding_level) * encounter , 1, params@species_params$alpha, 
...
load("data/full_20200502_NoDiffusion_SaveTimeStep.RData")
zoomssgrid <- res[enviro$CellID]
...
testdf$zoomizertcb[i] = sum(zoomizergrid[[i]]@n[tmax,,]*w)
}
...
fit <- lm(log10(testdf$zoomizertcb)~log10(testdf$zoomsstcb))
