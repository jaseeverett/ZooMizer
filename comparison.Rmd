...
We want to check that the initial setups were all the same:
...
#abundances - phyto
all.equal(zoomsstest$model$nPP/zoomsstest$model$param$w_phyto ,zoomizertest@n_pp[1,1:length(zoomsstest$model$nPP)], check.attributes=FALSE)

#abundances - zoo and fish
all.equal(zoomsstest$model$N[1,,] / matrix(zoomsstest$model$param$w,nrow=zoomsstest$model$param$ngrps, ncol = length(zoomsstest$model$param$w), byrow=TRUE), zoomizertest@params@initial_n, check.attributes=FALSE)
...
all.equal(zoomsstest$model$N[1,,] / zoomsstest$model$param$w, initialnsaved, check.attributes=FALSE)

# quick check here: abundances after a time step?
all.equal(zoomsstest$model$N[2,,] / zoomsstest$model$param$w, zoomizertest@n[2,,], check.attributes=FALSE)
```
...
all.equal(zoomizeregrowth/zoomizertest@params@w, gg, check.attributes=FALSE) #it was really close earlier with zoomizeregrowth/w, now mizer's N is in 1/g so perhaps this is the right conversion?
```
...
which(zoomsstest$model$M_sb != zoomizertest@params@mu_b)

# senescence mortality seems to differ at the maximum sizes - rounding error in sizes?
