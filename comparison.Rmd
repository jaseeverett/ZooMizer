...
all.equal(zoomsstest$model$nPP ,zoomizertest@n_pp[1,1:length(zoomsstest$model$nPP)] * zoomizertest@params@dw_full[1:length(zoomsstest$model$nPP)], check.attributes=FALSE)
...
all.equal(zoomsstest$model$N[2,,] , sweep(zoomizertest@n[2,,],2, zoomizertest@params@dw, "*"), check.attributes=FALSE)
...
If you look at the relevant comparisons (`a` against `A` and `b` against `C`), it's apparent that the Mizer values are around an order of magnitude too high. This might mean that something has been counted twice in the ZooMizer code?
...
zoomssparams <- params
zoomssparams@initial_n_pp[1:71] <- zoomsstest$model$nPP / params@dw_full[1:71]
...
zoomssparams2 <- params
zoomssparams2@initial_n_pp[1:71] <- zoomsstest$model$nPP
zoomssparams2@initial_n[] <- zoomsstest$model$N[1,,]
e_growth3 <- new_EReproAndGrowth(params = zoomssparams2, n = zoomssparams2@initial_n, n_pp = zoomssparams2@initial_n_pp, n_other = zoomssparams2@initial_n_other, t = 0, encounter = getEncounter(zoomssparams2), feeding_level = getFeedingLevel(zoomssparams2))

#View(e_growth3) #even worse

zoomssparams3 <- params
zoomssparams3@initial_n_pp[1:71] <- zoomsstest$model$nPP / params@dw_full[1:71] / params@dw_full[1:71]
zoomssparams3@initial_n[] <- sweep(sweep(zoomsstest$model$N[1,,], 2, params@dw, "/"), 2, params@dw, "/")
e_growth4 <- new_EReproAndGrowth(params = zoomssparams3, n = zoomssparams3@initial_n, n_pp = zoomssparams3@initial_n_pp, n_other = zoomssparams3@initial_n_other, t = 0, encounter = getEncounter(zoomssparams3), feeding_level = getFeedingLevel(zoomssparams3))

#View(e_growth4) #too far - it isn't a factor of dw that it's off by.
...
all.equal(zoomsstest$model$M_sb, zoomizertest@params@mu_b, check.attributes = FALSE) # TRUE

#predation mortality
all.equal(M2, getPredMort(zoomizertest@params)[1,], check.attributes = FALSE) 

#overall mortality
all.equal(Z, mort, check.attributes = FALSE)
